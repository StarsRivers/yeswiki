...
if (!("noModule" in HTMLScriptElement.prototype)) {
  $("#actions-builder-app")
    .empty()
    .append(
      '<p>Désolé, votre Navigateur est trop vieux pour utiliser cette fonctionalité.. Mettez le à jour ! ou <a href="https://www.mozilla.org/fr/firefox/new/">installez Firefox</a> </p>'
    );
} else {
  window.myapp = new Vue({
    ...
    computed: {
      actionGroup() {
        return this.currentGroupId
          ? this.actionGroups[this.currentGroupId]
          : {};
      },
      actions() {
      ...
      configPanels() {
        ...
        if (
          ...
          Object.values(this.selectedAction.properties).some(
            (conf) => conf.type
          )
        ) {
          result.push({
        ...
        for (let actionName in this.actions) {
          if (actionName.startsWith("common"))
            result.push({ params: this.actions[actionName] });
        }
        return result;
      },
      ...
      selectedActionAllConfigs() {
        let result = {};
        this.configPanels.forEach(
          (panel) => (result = { ...result, ...panel.params.properties })
        );
        return result;
      },
      wikiCodeBase() {
    ...
    methods: {
      initValues() {
        ...
        if (this.isEditingExistingAction) {
          ...
          if (newActionId in ACTIONS_BACKWARD_COMPATIBILITY)
            newActionId = ACTIONS_BACKWARD_COMPATIBILITY[newActionId];
        ...
        if (this.loadedForms[this.selectedFormId]) {
          ...
          $.getJSON(
            this.baseUrl +
              `root/json` +
              firstSeparator +
              `demand=forms&id=${this.selectedFormId}`,
            (data) => {
              ...
              if (!this.selectedForm && this.isEditingExistingAction)
    ...
    mounted() {
      $(document).ready(() => {
        ...
        new FlyingActionBar(
          this.editor,
          this.actionGroupsWithBackwardCompatibility()
        );
